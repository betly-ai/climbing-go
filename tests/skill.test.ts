...
describe('skill package', () => {
  ...
  it('documents the skill entrypoint in the README', async () => {
    ...
    expect(readme).not.toContain('climbing-go order');
    expect(readme).not.toContain('POST <api_base_url>/api/climbing-go/auth');
    expect(readme).not.toContain('"type": "streamablehttp"');
    expect(readme).not.toContain('"Authorization": "Bearer ACCESS_TOKEN"');
    expect(readme).not.toContain('payment_channel');
