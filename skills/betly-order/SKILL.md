...
- 如果用户没有给数量，默认数量为 1
- 必须由调用方或用户提供 `org_id`、付款手机号或已加密手机号、`api_key`、`api_secret` 和 `secret_version`；不要自行猜测组织或手机号
...
`order preview/create` 会先通过登录 MCP 换取短期 `access_token`，再调用下单 MCP。`--quantity` 不传时默认 `quantity = 1`，数量必须为正整数。

## Query Strategy

1. 用户已经给出 `org_id`、手机号或已加密手机号、接入方密钥参数、`store_id` 和 SKU 时，先预览订单
...
- 返回登录失败、用户不存在或手机号错误时，请用户确认手机号、组织上下文和接入方密钥参数
